...
#define TOTAL_MAX_ARGS 7		// initially set to 60
...
int tracepoint__syscalls__sys_exit_execve(struct trace_event_raw_sys_exit* ctx)
{
	...
	bpf_perf_event_output(ctx, &exec_events, BPF_F_CURRENT_CPU, event, sizeof(struct event));
	// size_t len = EVENT_SIZE(event);
	// if (len <= sizeof(*event))
	// 	bpf_perf_event_output(ctx, &exec_events, BPF_F_CURRENT_CPU, event, len);
cleanup:
